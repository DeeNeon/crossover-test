...
//Initializing interface object of this model.
var userModel = {};
var pwd = '5f4dcc3b5aa765d61d8327deb882cf99';

//seeding database with default users
userModel.seed = function(){
	var defaultUser = new User({username:'ali', password:pwd, activeSession:''});
	defaultUser.save(function(err, user) {
	  if(err) console.dir('error occurred in populating database' + user);
	});

	defaultUser = new User({username:'harry', password:pwd, activeSession:''});
	defaultUser.save(function(err, user) {
	  if(err) console.dir('error occurred in populating database: ' + user);
	});

	defaultUser = new User({username:'tom', password:pwd, activeSession:''});
	defaultUser.save(function(err, user) {
	  if(err) console.dir('error occurred in populating database' + user);
	});
